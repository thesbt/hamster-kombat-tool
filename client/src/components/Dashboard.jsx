...
function Dashboard({ setIsAuthenticated }) {
  ...
  const handleAddCard = async (e) => {
    ...
    const token = localStorage.getItem("token");
    try {
      await axios.post("http://localhost:3000/api/admin/cards", newCard, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchAllCards();
  ...
  const confirmAdminDeleteCard = async () => {
    ...
    try {
      await axios.delete(
        `http://localhost:3000/api/admin/cards/${cardToAdminDelete.id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const fetchUserCards = async () => {
    ...
    try {
      const response = await axios.get("http://localhost:3000/api/user-cards", {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleEditCard = async (e) => {
    ...
    try {
      await axios.put(
        `http://localhost:3000/api/user-cards/${cardToEdit}`,
        {
  ...
  const handleDeleteCard = async () => {
    ...
    try {
      await axios.delete(
        `http://localhost:3000/api/user-cards/${cardToDelete}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
